...
public class ArgumentParser {
	...
	public void parse(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			// if flag, set prevFlag
			if (isFlag(currentToken)) {
				prevFlag = currentToken;
				valueExpected = true;
				value = null;
			}
			...
			// add/update flag in map
			if (prevFlag != null) {
				map.put(prevFlag, value);
			}
		}
	}
	...
	public Path getPath(String flag, Path backup) {
		if (map.get(flag) != null) {
			return Path.of(map.get(flag)).normalize();
		}
		return backup;
	}
	 ...
	 * @throws NumberFormatException in case
	 *
	...
	/**
	 * Returns the value the specified flag is mapped as a {@link URL}, or the
	 * backup value if unable to retrieve this mapping (including being unable to
	 * convert the value to a {@link URL} or if no value exists).
	 *
	 * This method should not throw any exceptions!
	 *
	 * @param flag   the flag whose associated value will be returned
	 * @param backup the backup value to return if there is no valid mapping
	 * @return the value the specified flag is mapped as a {@link URL}, or the
	 *         backup value if there is no valid mapping
	 *
	 */
	public URL getUrl(String flag, URL backup) {
		try {
	...
	/**
	 * Returns the value to which the specified flag is mapped as a {@link URL}, or
	 * {@code null} if unable to retrieve this mapping (including being unable to
	 * convert the value to a {@link URL} or no value exists).
	 *
	 * This method should not throw any exceptions!
	 *
	 * @param flag the flag whose associated value is to be returned
	 ...
	 *
	 */
	public URL getUrl(String flag) {
		return getUrl(flag, null);
	}
